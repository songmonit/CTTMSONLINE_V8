...
instance.web.CrashManager = instance.web.Class.extend({
    ...
    rpc_error: function(error) {
        ...
        if (error.data.name === "openerp.http.SessionExpiredException" || error.data.name === "werkzeug.exceptions.Forbidden") {
            this.show_warning({type: "Session Expired", data: { message: _t("Your CTTMS session expired. Please refresh the current web page.") }});
            return;
        }
    ...
    show_warning: function(error) {
        ...
        new instance.web.Dialog(this, {
            size: 'medium',
            title: "CTTMS " + (_.str.capitalize(error.type) || "Warning"),
            buttons: [
    ...
    show_error: function(error) {
        ...
        new instance.web.Dialog(this, {
            title: "CTTMS " + _.str.capitalize(error.type),
            buttons: buttons
 ...
 * If params.wait, reload will wait the CTTMS server to be reachable before reloading
 */
...
instance.web.UserMenu =  instance.web.Widget.extend({
    ...
    on_menu_help: function() {
        window.open('http://cttms.com/guides', '_blank');
    },
    ...
    on_menu_account: function() {
        ...
        if (!this.getParent().has_uncommitted_changes()) {
            ...
            P.call('get_param', ['database.uuid']).then(function(dbuuid) {
                ...
                instance.web.redirect('https://accounts.cttms.com/oauth2/auth?'+$.param(params));
            }).fail(function(result, ev){
                ev.preventDefault();
                instance.web.redirect('https://accounts.cttms.com/account');
            });
        }
    },
...
instance.web.WebClient = instance.web.Client.extend({
    init: function(parent, client_options) {
        ...
        this.set('title_part', {"zopenerp": "CTTMS"});
    },
    start: function() {
        var self = this;
 ...
 * CTTMS webclient in an iframe, using the Cordova framework.
 *
 * This class acts as a link between the webclient and the
 * CTTMS Android/iPhone App implemented with cordova.
 */
instance.web.Cordova = instance.web.Class.extend({}, instance.web.PropertiesMixin, {
    ...
    // cttms.send('foobar') in cordova will call messages.foobar()
    messages: {
        // launch the POS !
        pos: function() {
